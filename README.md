...
3. If the changelog lives somewhere other than `CHANGELOG.md` (e.g. towncrier
   writes to `docs/changelog.md`), set `release-changelog-path` — the action
   extracts the latest `<!-- towncrier release notes start -->` section
   itself. A `release-changelog-script` is only needed for project-specific
   changelog formatting beyond that extraction.
